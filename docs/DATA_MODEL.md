...
erDiagram
    ...
    Artifact {
        ...
        ArtifactType type
        string value "path, URL, branch name, or note file"
        timestamp created_at
    }
...
- `NOTE` — Path to a markdown note file the user maintains elsewhere (Obsidian vault, project folder, etc.)
- `CUSTOM` — User-defined freeform reference

Local-path types (`REPO`, `DIR`, `FILE`, `NOTE`) are stored as absolute
paths. Orbit does not own or create these files — it only tracks
references to files the user manages elsewhere, so a missing path is a
