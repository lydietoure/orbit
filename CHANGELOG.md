...
- `orbit link [id]` — attach external references to a work entry,
  ...
  `--file`, `--url`, `--note`, `--custom`); `--note <path>` links a
  markdown file you maintain elsewhere. `--remove` detaches a reference;
  with no type flag the linked references are listed. Local paths are
  stored absolute and a non-existent path is a warning, not an error.
  The id is optional and falls back to the selected entry. Deleting a
  work entry removes its artifacts.
