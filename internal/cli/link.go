...
//	orbit link [id] --note <path>        add a markdown-note artifact
...
func getCmdLink() *cobra.Command {
	// One string var per artifact type, plus the remove toggle. Binding
	// each flag to its own var keeps the surface declarative;
	...
	cmd := &cobra.Command{
		...
		Long: "Attach external references to a work entry, turning it into a hub " +
			...
			"file, url, note, custom). A note is a reference to a markdown file " +
			"you maintain elsewhere. Local paths are stored absolute and only " +
